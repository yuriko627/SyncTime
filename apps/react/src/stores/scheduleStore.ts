...
interface ScheduleState {
  ...
  // Meeting proposals
  createProposal: (proposal: Omit<MeetingProposal, 'id' | 'createdAt' | 'responses' | 'status'>) => void;
  respondToProposal: (proposalId: string, response: 'available' | 'maybe' | 'unavailable') => void;
  updateProposal: (proposalId: string, updates: Partial<MeetingProposal>) => void;
  ...
  getActiveUsers: () => User[];
  findCommonAvailability: (userIds: string[], date: string, duration: number) => { start: string; end: string; available: boolean }[];
}
  ...
  sync(
    (set, get) => ({
      ...
      loadUserCalendarEvents: async (userId) => {
        try {
          const response = await fetch(`http://localhost:5555/calendar/events?userId=${userId}`);
          ...
          if (data.success && data.events) {
            ...
            console.log(`Loaded ${data.events.length} calendar events for user ${userId}`);
          }
        } catch (error) {
      ...
      getEventsForUser: (userId) => {
        const { events } = get();
        return Object.values(events).filter(event => event.importedBy === userId);
      },
      ...
      findCommonAvailability: (userIds, date, duration) => {
        ...
        for (let time = dayStart.getTime(); time < dayEnd.getTime(); time += slotDuration * 60 * 1000) {
          ...
          const allAvailable = userIds.every(userId => {
            const userEvents = Object.values(events).filter(event => event.importedBy === userId);
