...
export class CalendarSyncService {
  ...
  async testConnection() {
    try {
      ...
      console.log(
        `Retrieved ${transformedEvents.length} events from primary calendar`
      )
  ...
  async syncCalendarEvents(
    ...
    try {
      ...
      console.log(
        `Successfully synced ${transformedEvents.length} events to ${documentPath}`
      )
   ...
   */
  async syncPrimaryCalendarWithOAuth(
    ...
    try {
      ...
      console.log(
        `Fetched ${googleEvents.length} events from user's primary calendar`
      )
      ...
      console.log(
        `Successfully synced ${userEvents.length} events to ${documentPath}`
      )
  ...
  async syncParticipantScheduleForEvent(
    ...
    try {
      console.log(
        `Syncing schedule blocks for participant ${participantId} in event ${eventId}`
      )
      ...
      const googleEvents = response.data.items || []
      console.log(
        `Fetched ${googleEvents.length} events from participant's calendar`
      )

      console.log("google event[0]: ", googleEvents[0])

      // Transform to privacy-focused schedule blocks
      const scheduleBlocks = transformGoogleEventsToScheduleBlocks(
        googleEvents,
        participantId
      )

      // Convert to document format
      const scheduleDocument = scheduleBlocksToDocumentFormat(
        scheduleBlocks,
        participantId,
        googleEvents
      )
      ...
      console.log(
        `Successfully processed ${scheduleBlocks.length} schedule blocks for participant ${participantId}`
      )
      ...
      console.error(
        `Error syncing schedule for participant ${participantId} in event ${eventId}:`,
        error
      )
      throw error
    }
  ...
  async getStoredEvents(calendarId: string): Promise<TonkScheduleEvent[]> {
    try {
      const documentPath = `calendar/${calendarId}/events`
      const eventsDocument = await readDoc<Record<string, TonkScheduleEvent>>(
        documentPath
      )
      ...
      console.log(
        `Retrieved ${events.length} stored events for calendar: ${calendarId}`
      )
  ...
  async getUserStoredEvents(userId: string): Promise<TonkScheduleEvent[]> {
    try {
      ...
      const events = Object.values(eventsDocument)
      console.log(
        `Retrieved ${events.length} stored events for user: ${userId}`
      )
        ...
        calendars: [
          {
            ...
            lastSync:
              primaryEvents.length > 0
                ? Math.max(...primaryEvents.map((e) => e.lastSyncAt))
                : null
          }
        ]
      }
